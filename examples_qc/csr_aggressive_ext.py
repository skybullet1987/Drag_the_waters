...
def apply_aggressive_120x_baseline(algo):
    ...
    algo.Debug(
        "AGGRESSIVE_120X baseline: maximize + hot vol 0.75/0.85/0.55, gross~1.35, SOXL/UVXY RSI."
    )
...
def apply_maximize_plus_baseline(algo):
    ...
    algo.Debug(
        "MAXIMIZE_PLUS: maximize signals, gross 1.15, vol 0.65/0.78/0.52 (between max and 120x)."
    )
    algo.maximize_backtest_equity = True
    algo._apply_maximize_backtest_equity_profile()
