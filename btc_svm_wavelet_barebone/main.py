...
class BtcSvmWaveletAlgorithm(QCAlgorithm):
    """
    This algorithm combines Support Vector Machines (SVM) and Wavelets 
    in an attempt to forecast the future price of BTCUSD on Kraken. First, it
    ...
    def initialize(self):
        ...
        self._leverage = self.get_parameter("leverage", 1)
        self._weight_threshold = self.get_parameter("weight_threshold", 0.005)

        self.settings.minimum_order_margin_portfolio_percentage = 0

        ticker = "BTCUSD"
        security = self.add_crypto(ticker, Resolution.MINUTE, Market.KRAKEN)
        security.window = RollingWindow[float](period)

        # Daily consolidator on the trade bars (crypto has no quote bars on Kraken via LEAN data).
        consolidator = self.consolidate(
            security.symbol, Resolution.DAILY, TickType.TRADE,
            self._consolidation_handler
        )

        # Warm up the RollingWindow with daily history.
